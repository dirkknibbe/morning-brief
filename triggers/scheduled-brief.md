...
Run `bun run fetch` via Bash. It writes the full payload to `data/fetch-<today>.json` and prints a compact summary to stdout: counts, per-source `health`, `failed_sources`, top-3 titles per source, and the file path. Read the JSON file with the `Read` tool to get the full items. Expect ~20-45 items total.
